...
- **OAuth 2.1 authorization server** (opt-in, for ChatGPT/Claude.ai web) — PKCE
  ...
  Skill surfaces are gated independently. ⚠️ **The static `MCP_AUTH_TOKEN`
  bearer carries both scopes by default**, so on that path the server-side flag
  is the only gate unless you narrow it: `MCP_AUTH_TOKEN_SCOPES=vault.read` makes
  it a read-only bearer (the variable can only narrow, and an unknown scope
  refuses to start). Analysed in
