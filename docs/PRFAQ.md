...
**Q. 私のノートは AI 提供企業の学習に使われますか？**
コネクタ側は vault を一切アップロード・常時同期しません。渡すのは**検索でヒットした必要分だけ**で、しかも既定は read-only です。渡した先（ChatGPT / Claude など）での学習利用は各 AI のポリシー次第なので、機微なノートは各 AI のデータ設定（学習オプトアウト等）と併用してください。「丸ごとクラウドに置かれることはない」のが本製品の保証範囲です。

**Q. 料金はかかりますか？**
コネクタ自体は OSS（公開リポジトリ）で無料です。費用が出るのは各 AI 側の利用料と、web 連携する場合の独自ドメイン/ホスティング（任意）だけです。ローカル利用なら追加コストはほぼありません。

**Q. オープンソースですか？自分で監査できますか？**
はい。コードは公開リポジトリで、誰でも読めます。一方で**あなたの vault は非公開**のまま（`.gitignore` で除外、テストは合成データのみ）。「コードは公開・ノートは非公開」が設計の核です。
