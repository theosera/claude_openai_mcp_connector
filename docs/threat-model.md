...
| Serving an attacker-authored note in place of the one a caller asked for                     | Same layer 2 as above. Untrusted vault content cannot claim another document's identity without the collision being detected — including the case where the forged `id` is the victim's own vault-relative path (`INV-2`, `INV-5`).                                                                                                                                                       | Files written verbatim through the constrained surfaces (`append_audit_report`, `compare_and_swap_audit_state`, Skill `references/*.md`) can still *author* an `id`; refusing to honour it is the current control, rejecting it at write time is tracked in [`ROADMAP.md`](./ROADMAP.md) as an `INV-8`/`INV-9` change. |
