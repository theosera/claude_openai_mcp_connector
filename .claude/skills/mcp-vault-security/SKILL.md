...
- **経路①は依存更新で直らない (gray-matter 自身のコード)。経路②は直る。**
  ...
  **古い js-yaml が許容可能に見える**。
  ...
  _"CVE-2026-59870 fix not backported"_ なのに、同じレコードの `patched_versions` が `>=3.15.1`。
...
- **上限は実データで決める** (実 vault 2,381 ノート / median 225 B / max 1,042 B → 8 KiB で 7.9 倍の余裕)。
  ⚠️ **この倍率は as-of 付きの値で、cap でなく「最大のノート」の性質なので、誰もコードを触らなくても腐る。**
  **2026-09-01 再測** (その日サーバが索引していた 2,801 files / frontmatter 保持 2,462 / ガードと同じ数え方。
  ⚠️ **件数は 1 時間のうちに 181 動いた** — これは「この標本の分母」であって「vault の大きさ」ではない):
  median 272 B / max **1,771 B** → **4.6 倍**。over-cap 0 件・unterminated 0 件なので **cap は据え置き**。
  ⇒ **倍率を引く前に最大値を測り直す。**
...
- **超過は必ず出力する。** read 経路は `parseError` → ファイル名付きで stderr、本文のみ index。
  ...
  **★ これは「書く直前の parse」だけでなく「書く内容を組み立てる parse」にも効く。**
  `planUpdate` は現本文を parse して patch をマージするので、**`parseMarkdownSafe` ではなく
  `parseMarkdown`** を使う (2026-08-11 の外部レビュー指摘)。safe 版は失敗時に**空 frontmatter へ
  縮退**するので、壊れた YAML / 上限超過のノートを更新しようとすると**読めなかった全フィールドを
  削除する diff** が組み上がり、承認者にはそれが意図した変更に見える。**read は「そのノートを
  返し続ける義務」があるから縮退してよい。writer にその義務は無い。**
...
- **★ 上書きする唯一の write は atomic に置換し、直列化する** (`src/atomicWrite.ts` の
  `replaceFileAtomically` = 同一ディレクトリ temp → 所有者/権限を復元 → rename)。in-place
...
- **★★ 置換は「書き込み」と権限要件が違う。3 つとも弱めない**:
  1. **要るのはディレクトリの write 権限**で、ファイルの所有権ではない。in-place はファイル
     権限だけで足りていたので、**共有ディレクトリでは挙動が変わる**。エラーは**どの権限が
     足りないかを名指しする** (生 `EACCES` を出さない)。⚠️ **in-place へフォールバックしない** —
     それが塞いだはずの torn write そのもの。
  2. **所有者を復元できなければ rename しない (fail closed)。** rename は**対象を所有していなくても
     ディレクトリ write だけで通る**ので、他ユーザのノートを自分所有の temp で置換でき、
     `chown` を戻せないと**静かに所有者が移る**。「その所有者になれないなら元々置換もできない」は
     **偽** — 2026-08-11 の外部レビューで指摘され、実際に誤っていた。temp の uid/gid が
     **既に一致するときだけ** `chown` を省く。
  3. **temp は handle 経由で stat/chown/chmod する** (path で check→use しない)。作成は
     `mode` 付き `wx` → 完了後に `chmod` で厳密化 (open flag は umask に削られ、`chmod` だけだと
     **書込中に既定 mode で存在する窓**ができる)。失敗時は**どの経路でも** temp を掃除する。
...
4. **read-only 既定 + per-request 解決** — write tool は対応する許可がないとき
   **registerTool 自体を呼ばない** (discover もさせない)。document write は `MCP_HTTP_ALLOW_WRITE`、
   ...
   document write を出さない。stdio は `serveStdio` で両 era を同一 factory から提供するが、
   **「full」なのは二段階の document write だけ**で、legacy create (INV-3) / Skill (INV-8 item 1) /
   audit (INV-9 item 1) は**それぞれ独立の stdio opt-in**を要る。**ここに写しを置かない。**
...
5. **token は opaque 256-bit + rotation + audience/scope 束縛** — access/refresh とも CSPRNG、
   ...
   わざと生かし、その再提示で**自世代より上の**家系を revoke する。⛔ **提示 record 自身と
   同世代の access token は残る** — `revokeFamilyAbove` は `generation >` だけを消す。
   ⚠️ 60 秒は**上限**で固定ではない (`Math.min` で残 TTL に切り詰まる)。下の 7 を見よ)。token は **canonical resource `${issuer}/mcp`
...
7. **state 永続化 (opt-in `MCP_OAUTH_STATE_FILE`) は hash-at-rest + 完全性検証 + fail-closed** —
   ...
   **永続化しない** (60s 単回)。**`rotateRefreshToken` の 3 アーム** (期限切れ / 未 rotate の
   ...
   ⚠️ **単回性は維持していない** — 回転した refresh は 60 秒の replay grace の間わざと
   生き残る (上の 5)。**この 1 文は「即 disk 反映して単回性を再起動越しに維持」と書いていた**
   が、grace window が入った時点で後半だけが偽になった。**前半は主語を絞れば今も真。**
   ⭐ 訂正の根が「即時失効」でなく「有効範囲を書かなかったこと」である点を残す。
   load 時に期限切れを drop。⛔ **save 失敗は握り潰される** (警告のみで auth を壊さない) ので、
   「即 disk 反映」が保証するのは**試みた**ことであって、着地したことではない。
...
1. `MCP_SKILLS_SUBDIR` は primary vault 配下の安全な相対パスに限定し、起動時に containment と
   ...
   **★ 予約と tool 登録は別の決定 — 全 transport で独立 opt-in が要る** (INV-9 item 1 と同型)。
   `MCP_SKILLS_SUBDIR` は「このサブツリーを一般 write から予約する」だけ。Skill write tool を
   **登録**するのは HTTP = `MCP_HTTP_ALLOW_SKILL_WRITE` / stdio = **`MCP_STDIO_ALLOW_SKILL_WRITE`**
   (既定 off、`src/config.ts` の `stdioAllowSkillWrite` → `src/index.ts` の `allowSkillWrite`)。
   stdio は以前 `Boolean(skillStore)` で登録していたため、**「write 可能な全 process に subdir を
   設定せよ」という運用指示に従うと、untrusted vault content を読む interactive セッションに
   `plan_skill_create` が生えた** (#113)。予約は `config.skillsSubdir` → `createStore` 経由なので、
   **tool を外しても INV-8 は効き続ける**。stdio 起動行は 3 状態を出す:
   `skills=off` (subdir 無し = 予約なし) / `skills=reserved-only` (既定) / `skills=on`。
   `MCP_STDIO_ALLOW_SKILL_WRITE=1` で subdir 未設定なら**起動拒否**。
...
| `src/knowledgeStore.ts`                                                                                                                                                                                                                                                                                                          | INV-1,2,3     | `walkMarkdownFiles`/`resolveForWrite`/`resolveForExistingRead` は realpath 照合必須。**`readDocument` も例外ではない — cache を見る前に毎回 realpath + `relativeToRoot` を通す** (INV-1 item 9 が正典)。#108 で「呼び出し側 (walk / resolveForWrite / resolveForExistingRead) が同じ呼び出しでガードを通しているので**解決済みパスの二重解決**」として外したが、**マージ前に revert した**。「同じ呼び出しで」は「**同じ瞬間に**」ではなく、walk がパスを集めてから読むまでの窓で親ディレクトリを root 外へ移して symlink で戻すと、**子ファイル自身の `dev`/`ino`/`ctime`/`mtime` は全て不変**なので (親の rename は子の ctime を動かさない)、stat signature は**本物の escape をまたいで一致する**。**`dev` を落とさない** — `ino` は 1 filesystem 内でしか一意でないため。ただし `(dev, ino)` は**鮮度用**であって封じ込めの代わりではない (#108 はそこに載せようとして戻した)。**新しい呼び出し側を足すときも自分でガードを通す** — `readDocument` の再解決は最後の砦であって、呼び出し側のガードの代替ではない。`applyPlannedUpdate` の stale sha と `applyPlannedDocumentCreate` の content sha・confirmed path 照合を消さない。create parent は component ごとに symlink を拒否。scan は `mapWithConcurrency` で FD 上限を絞り、`readDocumentResilient` は **transient FS code (EAGAIN/EMFILE/ENFILE) だけ** retry。containment throw や読取不能は**握り潰さず skip (root 外を絶対に返さない)** — fail-closed を弱めない。**INV-9**: 監査サブツリー予約 (`assertNotAuditReserved`) を `resolveForWrite`(create の共通 choke)/`applyPlannedUpdate`(update・権威)/`planUpdate`・`validateCreateTarget`(早期) で通す (realpath 照合)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `src/index.ts`                                                                                                                                                                                                                                                                                                                   | INV-6         | transport 選択のみ (`selectedTransport`)。stdio=`serveStdio(factory, {legacy:'serve', onerror})` で両 era (⛔ 面は full ではない — Skill / audit / legacy create は個別 opt-in。INV-8 item 1 / INV-9 item 1 / INV-3) / http=`buildMcpServer` + `startHttpServer`。`legacy` は既定に頼らず明示 (既定が動くと 2025 client を黙って締め出す)。`onerror` を外さない — `serveStdio` は wire の start 失敗を**握り潰す**ので、無いと起動失敗が "ready" 行だけの成功に見える。`onerror` は **error class のみ**出す (同じ callback が受信バイトを引用しうる実行時エラーも受ける)。token/本文をログに出さない。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
> **★★ 逆検証そのものが空振りする。「mutation を入れた」と「mutation が届いた」を別々に確認する。**
> **実測: #122 で 2 件・#125 で 4 件、入れた mutation がそもそも当該分岐に到達しなかった** —
> **整形でパターンが一致しなくなった** / **狙いと違う最初の一致に当たった** /
> **定数が両辺に現れて相殺した** / **呼び出しが読まなくなったフィールドを読んだ**。
> **6 件とも「ガードが効いていて緑」と見分けがつかない。** 逆検証を実施した記録だけが残り、
> 実施できていない状態になる。
> ⚠️ **`GD-VACUOUS-CHECK` の系だが、対象が 1 段ずれている** — あちらは**テストがガードに
> 到達したか**、こちらは **mutation がガードに到達したか**。
> **検査を検査する手続き自身が vacuous になりうる**、ということ。
> ★ **唯一効いた形**: mutation を入れた版で**狙った assert が実際に赤くなったこと**を確かめ、
> **その確認をテスト側に書き残す** (#122 / #125 はどちらもそうした — 次に逆検証する人は
> その形を真似ればよい)。
> ★ **届いた mutation は、ガードの不在以外のものも露出させる** — #125 では届いた 2 件が
> **本物の穴**を出した (`recent_docs` の中身が何も pin されていなかった /
> include の検査が「無視された場合」と区別できていなかった)。
> **赤が出たら「pin できていた」で終えず、「何が pin できていなかったか」を読む。**

> **★ 逆検証が vacuous になりやすい 3 つの形。設計の時点で外す** (テストを書いてからでは遅い)。
> **① 既定を変えない変更は、既定経路のテストでは何も証明しない** — ガードを外しても全部緑になる。
> **狭めた設定を明示的に踏む経路**でテストし、**捕捉そのもの**を assert する
...
> **実測: CodeQL が #106 / #107 / #108 で同じ形を 3 回連続で検出した** (いずれも新規テスト、high)。
> **3 回とも「このルールを読んだ上で」踏んでいる** — 規約として覚えるのでは足りない。
> **テストで同じパスに 2 回触れそうになったら、その時点で handle にする**のが唯一効いた形。
...
- **scan の絞り込み (`path_prefix` → walk)**: prefix 付き検索が**絞らない検索と同じ結果**を返し、かつ
...
|            | (a) `/claude-security` (plugin)                                     | (b) `/security-review` |
| ---------- | ------------------------------------------------------------------- | ---------------------- |
...
**実測 (2026-08-11 JST)**: `atomicWrite.ts` を足した際、temp を既定 mode で作ってから `chmod`
していたため **`0600` のノートが書き込み中だけ `0644` で存在**した。**CI 7 step 全緑・逆検証
4 ガード分パスの後で、(b) だけがこれを出した。**

⚠️ **それでも逆検証の代わりにはならない。** 同じ PR で、mtime を戻す cache テストが**ガード無しでも
緑**だった件は**レビューでは出ず、逆検証だけが**出した。**レビュー = 書かなかったガード /
逆検証 = 書いたガードが効くか。**

⚠️ **走らせる前に diff が空でないことを確かめる。** **実測**: `origin/HEAD` 未設定で
`git diff origin/HEAD...` が全部エラーになり、**空 diff をレビューして「指摘なし」になる寸前**
だった (`git remote set-head origin main` で復旧)。cache テストが vacuous に緑だったのと**同じ
失敗モード** — 検査した結果か、**検査に到達しなかった結果か**。

⚠️ **どちらの分岐も、外部レビューの代わりにはならない。** 2026-08-11 に (b) を通した PR へ、
外部レビューが **P2 を 2 件**足した (write 計画の parse 縮退 / 所有者を戻せないまま rename)。
**どちらも「書かなかったガード」で、(b) が拾える形だったが拾わなかった。**
