...
| `src/config.ts`                                                                                                                                                                                                  | INV-1,4,6,7   | secret は env のみ。`loadHttpConfig`/`loadOAuthConfig` は token/issuer/password 未設定で fail-closed。bind 既定 loopback。 **★ サーバ state を vault の中に置かせない** — `MCP_OAUTH_STATE_FILE` (registered client 一覧・salt・HMAC タグ) と明示指定の `MCP_PATCH_STATE_DIR` (staged plan = 文書の全文) を全 knowledge root に対して boot で照合する (`assertOutsideKnowledgeRoots`)。**root は read surface** なので、中に置くと index されて `search`/`fetch` から読める。**文字列 prefix 比較に退行させない** — 存在する最深の祖先を realpath してから比較する (vault へ向く symlink 経由は prefix を共有しない)。`MCP_ENV_FILE` は**対象外** (root より前に読まれるため / 別機構が要る) — 半端に当てず未処理として残す。                                                                                                                                                                                                                                                                                                                                                                                       |
...
- **サーバ state の置き場所 (根 E の隣 / B-3)**: `MCP_OAUTH_STATE_FILE` が vault 内 → boot で reject し
  **どのルートかを名指し** / **vault へ向く symlink 経由**も reject (prefix を共有しないので
  文字列比較では通ってしまうケース) / 二次 read-only root でも reject / vault 外は従来どおり通る /
  root 未設定で state file を指定 → **fail closed** (照合対象が無い) / 明示 `MCP_PATCH_STATE_DIR` も同様。
  ★ 逆検証は**呼び出しサイトごとに 1 回ずつ** — OAuth 側を外すと 4 本、patch-state 側を外すと 1 本が
  赤くなり、**集合が交わらない** (片方の緑がもう片方の証拠にならないことの実測)
