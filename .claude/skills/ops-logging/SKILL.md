...
- **PEM 秘密鍵**: BEGIN / END の marker 行を置換し、その範囲 (BEGIN 〜 END、または
  ...
  **本物の redaction が出すのと同じ token** なので、**壊された内容は「通常のマスク処理」に
  ...
  2 本の行全体 catch-all が範囲なしで取り、上限の費用は無い。sed の形は入れ子:
  `/BEGIN/,+100{ /BEGIN/,/END/ {…} }` — 外側が数え、内側は END で止まるので 100 は到達の
  天井であって床ではない。`,+N` address も入れ子ブロックも GNU / BSD sed 両方にあり、
  CI (GNU) と操作者の shell (BSD) の両方でテストが走る。
