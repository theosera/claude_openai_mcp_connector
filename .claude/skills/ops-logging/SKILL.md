...
- **引数位置の資格情報** (2026-09-24 追加・S-1 F6): `-u` / `--user` の `name:secret`、MySQL 系
  ...
  #232 で戻した** (二重引用・単引用の 1 本ずつ・閉じ引用符まで・bare の直前。#231 では #186 と一緒に外していた・(c))。
...
- **反復した `-p` / `-a` を全部マスクする** (2026-09-25・#234 の形 3・owner 決定「案 A」): 貪欲な語の並びが前の flag を
  ...
  **先頭部分だけの値** (`*` / `**`) と、それに**ハイフンが続く値** (`*-` / `***M-`) は一度も当たらず丸ごと平文になった
  ...
  ⚠️ **終わりには 5 連ハイフンも数える** — 値の文字クラスは 5 連ハイフンを跨げないので、`-p*-----` / `-p***MASK-----rest`
  はこの 1 本にもループの規則にも当たらず丸ごと平文になった (main は 5 連の手前をマスクしていた・#243 の写しに付いた
  Codex の指摘・実測で再現)。5 連から先は main と同じく残る。
