...
- **反復した `-p` / `-a` を全部マスクする** (2026-09-25・#234 の形 3・owner 決定「案 A」): 貪欲な語の並びが前の flag を
  呑み、最後の値しかマスクされなかった (`mysql -p<A> -p<B>`)。2 規則を `:m`…`tm` / `:r`…`tr` のループで回し、各回は
  **`g` 付き** (付けないと 1 回 1 件で行頭から探し直し、1 行に N 個の client で O(N²)・最初の綴りで `redis-cli -a x` ×
  4,000 が BSD sed 195 s)。`***MASKED***` の 12 文字で始まる値は値と見なさない (その 1 つの接頭辞の補集合 —
  `***abc` / `*abc` は値のまま) ので自分の出力に再一致せずループが終わる。補集合は先頭部分の後に 1 文字を要するので、
  **先頭部分だけの値** (`*` / `**`) と、それに**ハイフンが続く値** (`*-` / `***M-`) は一度も当たらず丸ごと平文になった
  (main は少なくともハイフンの手前をマスクしていた・#243 の Codex P1・実測で再現) ⇒ 各ループの中に専用の 1 本を足し、
  その値で終わるときにマスクする (終わらせた文字は戻す)。12 文字の marker には当たらないのでループは終わる。
  ⭐ **ループの中は専用の印 `***MASKEDP***` に置き換え、除外もこの印に掛ける**。2 つ目のループの直後の 1 本 (区切りは `#` —
  共有リダクタの `s/<形>/***MASKED***/g` の持ち上げに印を秘密の形として読ませないため) で `***MASKED***` に戻すので、
  印は mask() の外に出ない (テストで pin)。⛔ 除外を marker そのものに掛けた版は、前段の token の規則 (`sk-` / `AKIA` …) や
  範囲内の連なりの規則が値の**先頭だけ**を `***MASKED***` にした値を拒み、残りを平文で出した (#243 の change scan r4
  F1 / F2・実測で再現)。⚠️ 代償: 値が文字どおり `***MASKEDP***` で始まる秘密はループの規則ではマスクされない (テストで pin)。
  ⛔ **引用値の空白 (形 1・2) と引用された引数の中の `-price` (形 4) はこの変更では扱わない** — shell の語として読む形を
  一度作り、2 回目の change scan が 7 件 (JSON の `\"`・`'\''`・fallback が消した閉じ引用符・閉じない引用符・長さ
  上限の無い引用の断片で 2 乗) を出した。どれも main より平文を増やした (実測)。⇒ #232 で、escape を読む・閉じない
  引用符では main の規則に落とす・引用の断片に長さの上限、を最初から満たす形で扱う。`-u` は main のまま (curl に
  anchor すると継続行と 13 語以上の curl が漏れた・1 回目の change scan)。
