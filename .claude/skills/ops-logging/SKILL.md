...
- **引数位置の資格情報** (2026-09-24 追加・S-1 F6): `-u` / `--user` の `name:secret`、MySQL 系
  ...
  ⛔ この系列が足した規則 8 本は**すべて address `/```|~~~/!` を持つ** — フェンスの連なりを含む行では動かない。
...
- **check-then-append** (2026-09-24 追加・A-47 F2): `grep -q "token: " f || echo "token: V" >> f` —
  ...
  (fallback は byte 単位で不変)。⛔ この pass は**引用符の直後にある keyword にだけ**当たる — 当たり先を
  絞らないと `--key` の中の `key` に当たって後続の `token:` を値として消し、fallback が最後の値を
  見られなくなった (`token: "--key token: V` で V が残った・#231 のレビュー指摘・実測で再現)。値の先頭文字から `=` `:` を外してある — 外さないと `key === "x"` の
  `=` を 1 文字の値として食う (追跡ファイルの差分で検出)。
- **YAML の二重アポストロフィ** (2026-09-24 追加・#186 / A-57): `'pre''fix'` の `''` を、**既にマスクした
  値の直後に限って**続きとして読む (単引用値の対の直後の 1 本・dash 境界付き)。⛔ **単引用値の文字クラスに
  `''` を足してはいけない** — 最長一致で「閉じ引用符 + 迷子の `'`」から**次の keyword の開き引用符**まで走り、
  2 つ目の値の 2 語目以降が平文で出る (この変更の change scan F2・実測で再現)。
  ⚠️ 残渣: `''` の後に 5 連ダッシュがある値はそこから残る (変更前も `''` を読まなかったので同じ)。
  続きは**空白・`:`・`=` で止まる** — fallback より前に走るので、区切りを越えられると引用の打ち間違いで
  `' ; password: '` を呑み、fallback が keyword を見られなくなった (3 回目 F3)。⚠️ 残渣: `'it''s a test'` は
  ` a test` が残る (変更前と同じ)。
