...
| T9 | OAuth 2.1 flow abuse for the web-client path (auth-code interception/replay, PKCE downgrade, open redirect, weak/guessable tokens, unbounded client/token growth) | PKCE **S256 mandatory** (`plain` rejected); authorization codes are CSPRNG, **single-use**, short-TTL, and bound to client_id/redirect_uri/code_challenge; redirect URIs **exact-match** a registered https/loopback value (no open redirect); login-password gate is constant-time + **fail-closed** (refuses to enable OAuth without issuer URL + password); tokens are 256-bit opaque with refresh rotation; all OAuth collections are capped + pruned; no codes/tokens/passwords logged. | `src/oauth/pkce.ts`, `src/oauth/store.ts`, `src/oauth/provider.ts`, `src/config.ts`, `src/httpServer.ts` |
