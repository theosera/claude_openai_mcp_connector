...
### What CODEOWNERS does, and what it does not

`.github/CODEOWNERS` **routes** review: it decides who is requested. Whether
anything **blocks** a merge is a property of the branch ruleset, not of that
file. Read the ruleset — do not infer it from CODEOWNERS or from any prose,
...
`active`): `require_code_owner_review` is `true`, but
`required_approving_review_count` is `0` and `bypass_actors` grants
`RepositoryRole` 5 `bypass_mode: always`. So ownership of `.github/` routes the
...
- **§3.1 / §3.2 / §3.3 / §3.5 / §3.6 / §3.9 / §3.4 CI/CD** → SHA-pinned Actions,
  ...
  CODEOWNERS review routing, CodeQL.
...
```bash
pnpm test   # path traversal, symlink escape + cycle, frontmatter allowlists, stale-patch, overwrite collision, constrained Skill creation, constrained audit write surface (INV-9), HTTP bearer auth + tool-surface gates, OAuth 2.1
```
