...
進捗ノートや作業メモは、**PR に関する事実については GitHub のキャッシュ**である。
キャッシュを現在の状態として提示しない。同じ決定論的規律を鮮度にも適用する。

| 発火条件 (これをする前に)                                                                        | 必ずやること                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **担当表・残タスク・進捗表など「現在の状態」を提示する** / **次に着手するノード・PR・issue を選ぶ** | **(a) open PR と直近マージ済み PR を取得**して手元のノートと突き合わせる、**または (b) as-of を明記**して未確認と添える (例: `残り 2 ノード (メモ as-of 05:39 JST / GitHub 未確認)`) |

> ⚠️ **「必ず取得せよ」にしない。** 取得できない状況 (ネットワーク断・API 不通・権限無し) で
> 守れない規約は規約ごと無視される。**(a) か (b) のどちらか**なら常に守れる。
>
> **実測 (2026-08-10)**: 担当表を 10:32 に提示 → **90 分後**に該当 PR がマージ → 提示が陳腐化。
> **メモは正しく、読まれ、判断も正しかった** — 誤りは判断ではなく**有効範囲 (as-of) を
> 落としたこと**だった。as-of さえ付いていれば、あの提示は今も正しい。
>
