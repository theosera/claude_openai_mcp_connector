...
| 発火条件 (これをする前に)                                                                                                       | 必ずやること                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **調査・走査・悉皆確認・主張の検証・独立レビュー**に着手する前、または**サブエージェントのモデル階層を選ぶ**前 | `docs/subagent-delegation.md` を読む。要点は 2 つ — **(a) 現セッションの文脈に依存しない調査は、汚れていない文脈へ渡す** (蓄積した文脈は期待を供給し、期待こそ調査が試すもの) / **(b) 階層の軸は「難しさ」ではなく「見落としが痕跡を残すか」** — 悉皆・不在・一致を言う仕事だけ最上位のモデルへ寄せる |

> ⚠️ **渡した調査を自分でも並走させない。** 同じ期待から同じ地面を 2 回走っても相互検査にならない —
> **一致するので確認に見える**だけである。
>
> ⚠️ **設定できない値を「設定した」と書かない。** agent 起動ツールが取るのは model であって
> reasoning effort ではない (執筆時点。**現物のパラメータ一覧を見て確かめる** — この行はキャッシュ)。
> **実際より入念に走ったと報告された run は、素直に報告された run より悪い** — 次の読み手は
> その報告を信じて再実行しないからである。
...
| 発火条件 (これをする前に)                          | 必ずやること                                                                                                                                                                                                                |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **要約から復帰した最初のターンで、ツールを呼ぶ前** | ① **`.claude/settings.json` を読む** (一度も注入されない) ② **継続する作業に該当するスキルをロードし直す** (中身は残らない) ③ 再開するタスクを上の各発火表に**改めて**照合する (スキル / ROADMAP / 鮮度)。`CLAUDE.md` 自体は再注入されるが、**通し読みは安い保険**として併せてよい |
