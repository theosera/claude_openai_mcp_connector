...
>
> **突き合わせる窓を「直近」で濁さない。** 下限は上から順に採る:
>
> 1. **ノート自身の as-of**
> 2. **MCP 経由で書かれた vault 文書なら frontmatter の `updated_at`** — INV-3 でサーバが
>    所有する値なので、クライアントが書き換えられない = 下限として信用できる。
>    ⚠️ **「MCP 経由で」を落とさない。** サーバが stamp を押すのは `apply_planned_update` /
>    `apply_planned_document_create` を通った時だけで、**同じファイルを Obsidian や
>    エディタや `Edit` で直接書き換えても `updated_at` は動かない**。つまりこの値が
>    保証するのは「**この時刻以降に MCP 書込は無い**」であって「内容が古くない」ではない。
>    ⚠️ **実測 (2026-08-16)**: `patch-verification-log.md` を直接編集で更新したところ、
>    本文の as-of より `updated_at` が **83 分古い**まま残った。同じ turn で MCP 経由に
>    した `_state.md` は 2.4 分差、`_decisions.md` は 38 秒差で、**差の大小がそのまま
>    書込経路を言い当てていた**。**この規則を字義どおり適用すると、最初に外れるのは
>    この規則を書いている文書自身である。**
>    → **直接編集した文書では 2 を採らず 3 へ落ちる。** どちらで書いたか分からないなら、
>    分からないこと自体が理由なので 3 か 4 へ落ちる。
> 3. **git 追跡下のファイルなら、それを最後に変更したコミットの時刻**
> 4. どれも無ければ**窓が決まらないので (b) を選ぶ** — 比較の下限が決まらないまま
>    取得しても、確認したことにならない
>
