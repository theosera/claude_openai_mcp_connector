...
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **各席の残タスク表 (`<席 ID>-remaining-tasks.md`) を更新した**                                 | **統合監視セッション (1e / 次の統合監視) へ `SendMessage` で報告する。** ★ **報告に含める 3 点の本体は、その表自身のヘッダにある** (`_tools/build_day.py` が毎日生成する) — ⛔ **ここに写しを置かない** |

> ⛔ **この行が言えるのは「読ませる確率を上げる」までで、「守らせる」ではない。**
> ⭕ **だから本体は pull 側にある** — 統合監視が `build_day.py watch` で digest 差分を自分で検出する。
> ⇒ **報告が来なくても更新は見える。**
> **実測 (2026-09-04)**: 62 の `11,357→15,982 B` を、報告と独立に sha で検出した。
> ⚠️ **ただしこれを交差検証と呼ばない** — 62 が先に報告した後で回したので、示せたのは
...
> **実測 (2026-09-04)**: 18 が 09:21 に更新 → 取り込み済み → 09:35 の `[same]` を見た 18 が
...
> **実測 (2026-09-04)**: 本節を commit した直後、同じマシンの別席が**自分に注入されている
