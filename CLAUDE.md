...
長いセッションはコンテキストが要約 (compaction) に置き換わる。**要約は本ファイルの写しではない** —
発火表・ハードルールは「作業の経緯」ではないので要約の対象になりにくく、**最も落ちやすい部類**である。
落ちたことは自覚できない (無いものは思い出せない) ので、裁量ではなく発火で戻す。

| 発火条件 (これをする前に)                                                | 必ずやること                                                                                                                                          |
| ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **要約から復帰した最初のターンで、ツールを呼ぶ前**                       | `CLAUDE.md` と `.claude/settings.json` を**読み直す**。その上で、再開するタスクが上の各発火表に該当しないかを**改めて**照合する (スキル / ROADMAP / 鮮度) |

> **なぜ settings.json も読むのか。** deny ルール (secret 読取・自己改変) と hook 配線は
...
> 却下の理由はこちらには当たらない。**
>
> ⚠️ **スキルのロード済みは引き継がれない。** 要約は「このスキルをロードした」と書きうるが、
> **その中身は文脈から消えている**。該当する作業を続けるなら**ロードし直す** — 「ロードした記録」は
> ロードではない。
>
