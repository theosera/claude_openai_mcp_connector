...
> not part of any diff this job builds"。⇒ **commit 前に回すと常に空 diff** になり、
> ⛔ **指摘 0 件を「走査した」として記録してしまう**。⭐ 下の「空 diff を確かめる」は**この形も**指す。
> ⚠️ **実測 2026-09-10**: 4 席が独立にここで止まった。★ **文言と器具の食い違いを放置したまま
> 「注意して回す」で運用していたので、注意深い席ほど止まり、素通りした席は空振りを実施済みと記録しかけた。**
>
...
> なる事故が実際に起きかけた。⚠️ ⭐ **同じ症状を出す欠陥は 2 つある** — ★ その事故は
> `origin/HEAD` 未設定によるもので (skill 側に経緯あり)、⇒ **上の commit 境界の食い違いは別物で、
> しかも毎回起きる**。⛔ **注意書きが 1 つあったせいで、2 つ目が「既知の注意点」に見えて温存された。**
>
...
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **各席の残タスク表 (`<席 ID>-remaining-tasks.md`) を更新した**                                 | **統合監視セッションへ `SendMessage` で報告する。** ★ **報告に含める 3 点の本体は、その表自身のヘッダにある** (`_tools/build_day.py` が毎日生成する) — ⛔ **ここに写しを置かない** |

> ⛔ **この行が言えるのは「読ませる確率を上げる」までで、「守らせる」ではない。**
> ⭕ **だから本体は pull 側にある** — 統合監視が `build_day.py watch` で digest 差分を自分で検出する。
> ⇒ **報告が来なくても更新は見える。**
...
> `tanaoroshi` skill と統合監視の申し送り。**発火はファイル名の形だけで足りる。**
...
> ⇒ **発火表を足した当日は、走っている席へ別途通達する** (pull 側だけが当日から効く)。
>
